        ...
        <section className="relative py-16 sm:py-24 bg-gradient-to-b from-primary/10 via-background to-background border-b border-slate-200 dark:border-slate-800">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center space-y-4">
            ...
            <p className="text-base sm:text-lg text-slate-800 dark:text-slate-200 max-w-3xl mx-auto leading-relaxed font-medium">
              Founded on the pillars of academic excellence, ethical grounding, and forward-looking technological preparedness, <strong className="font-bold text-slate-950 dark:text-white">{schoolName}</strong> nurtures tomorrow&apos;s thinkers, creators, and compassionate citizens.
            </p>
          </div>
        </section>
        ...
        <section className="py-16 sm:py-20 bg-card">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-12">
              <div className="rounded-3xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/40 p-8 sm:p-10 space-y-4 shadow-sm hover:border-primary/40 transition-colors">
                <div className="h-12 w-12 rounded-2xl bg-primary/10 text-primary flex items-center justify-center font-bold">
                  <Compass className="h-6 w-6" />
                </div>
                <h3 className="text-2xl font-heading font-bold text-slate-950 dark:text-white">Our Vision</h3>
                ...
                <div className="space-y-2 pt-2 text-xs font-bold text-slate-900 dark:text-slate-100">
              ...
              <div className="rounded-3xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/40 p-8 sm:p-10 space-y-4 shadow-sm hover:border-primary/40 transition-colors">
                ...
                <h3 className="text-2xl font-heading font-bold text-slate-950 dark:text-white">Our Mission</h3>
                <p className="text-slate-800 dark:text-slate-200 text-sm leading-relaxed font-medium">
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
              {milestones.map((m, idx) => (
                <div
                  key={idx}
                  className="bg-card border border-slate-200 dark:border-slate-800 rounded-2xl p-5 space-y-3 relative hover:border-primary/50 transition-all hover:shadow-md"
                >
                  <div className="text-2xl font-heading font-black text-primary">{m.year}</div>
                  <div className="font-extrabold text-sm text-slate-950 dark:text-white font-heading">{m.title}</div>
                  <p className="text-xs text-slate-800 dark:text-slate-200 leading-relaxed font-normal">{m.desc}</p>
                </div>
              ))}
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {values.map((v, idx) => {
                ...
                return (
                  <div
                    key={idx}
                    className="bg-card border border-slate-200 dark:border-slate-800 rounded-2xl p-6 space-y-3 hover:shadow-lg transition-all shadow-sm"
                  >
                    <div className="h-12 w-12 rounded-xl bg-primary/10 text-primary flex items-center justify-center">
                      <Icon className="h-6 w-6" />
                    </div>
                    <h4 className="font-heading font-bold text-base text-slate-950 dark:text-white">{v.title}</h4>
                    <p className="text-xs text-slate-800 dark:text-slate-200 leading-relaxed font-normal">{v.desc}</p>
                  </div>
                );
              })}
            </div>
